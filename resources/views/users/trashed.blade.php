    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl leading-tight">
                Blony's {{ __('Deleted Users Management') }}
            </h2>
        </div>
    </x-slot>
            ...
            <section class="grid grid-cols-1 gap-4 px-4 mt-4 sm:px-8">
                ...
                <section class="min-w-full items-center bg-zinc-50 border border-zinc-600 rounded overflow-hidden">
                    ...
                    <table class="min-w-full text-left text-sm font-light text-surface dark:text-white">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center text-xl py-4">No deleted users found</td>
                            </tr>
                        @endforelse
                        </tbody>
                         ...
                         <tr class="text-black bg-zinc-100">
                            <td colspan="5" class="px-6 py-2">
                                @if( $deletedUsers->hasPages() )
                                    {{ $deletedUsers->links() }}
                                @else
                                    <p class="py-2 text-zinc-800 text-sm">All deleted users shown</p>
                                @endif
                            </td>
                        </tr>
                        </tfoot>
